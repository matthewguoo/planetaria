/* eslint-disable react-hooks/refs, react-hooks/purity -- legacy chart pattern:
   props are mirrored into refs during render so rAF/canvas draw callbacks read
   fresh values without re-subscribing, and a 2s tick re-renders through
   Date.now() to age the hold clock; predates the rules, verified in prod. */
/**
...
export function EnforcerBlock({ planId }: { planId: string }) {
  ...
  const bracketed = plan.tp_premium != null && plan.sl_premium != null;
  const span = bracketed ? Math.abs(plan.tp_premium! - plan.sl_premium!) || 1 : 1;
  const slDist =
    mark != null && plan.sl_premium != null ? ((mark - plan.sl_premium) / span) * 100 : null;
